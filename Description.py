#December lineage
# 9000,9005, 9004 and 9019 complete working with column and correct many to one attribute mapping 
import sys
import csv
import boto3
import pandas as pd
...
args = getResolvedOptions(
    sys.argv, ['S3_BUCKET', 'S3_OUTPUT_EXTRACT', 'MARQUEZ_URL'])

s3_bucket = args['S3_BUCKET']
# account_staging_9000 = args['S3_9000_KEY']
output_csv_path = args['S3_OUTPUT_EXTRACT']
marquez_url = args['MARQUEZ_URL']
account_staging_9000 = 'lineage-vr/account_rectypes/Account_9000_Staging.csv'
...
def process_csv(csv_string):
    ...
    for row in csv_reader:
        ...
        description.append(row['Details']
                               if 'Details' in row else None)

    return column_names, datatype, rec_type_values, mappings, join_conditions, description


# Updated logic for reading and processing the output CSV
csv_string_output = read_csv_from_s3(s3_bucket, output_csv_path)
column_names_output, datatype_output, rec_type_values, mappings, join_conditions, description_output = process_csv(
    csv_string_output)

# Read the input CSV and process it
csv_string_9000 = read_csv_from_s3(s3_bucket, account_staging_9000)
column_names_9000, datatype_9000, rec_type_values_9000, mappings_9000, join_conditions_9000, description_9000 = process_csv(
    csv_string_9000)

# Read the 9004 CSV and process it
csv_string_9004 = read_csv_from_s3(s3_bucket, account_staging_9004)
column_names_9004, datatype_9004, rec_type_values_9004, mappings_9004, join_conditions_9004, description_9004 = process_csv(
    csv_string_9004)

# Read the 9005 CSV and process it
csv_string_9005 = read_csv_from_s3(s3_bucket, account_staging_9005)
column_names_9005, datatype_9005, rec_type_values_9005, mappings_9005, join_conditions_9005,description_9005  = process_csv(
    csv_string_9005)

# Read the 9005 transpose CSV and process it
csv_string_9005_transpose = read_csv_from_s3(s3_bucket, account_transpose_9005)
column_names_9005_transpose, datatype_9005_transpose, rec_type_values_9005_transpose, mappings_9005_transpose, join_conditions_9005_transpose, description_9005_transpose = process_csv(
    csv_string_9005_transpose)

# Read the 9019 CSV and process it
csv_string_9019 = read_csv_from_s3(s3_bucket, account_staging_9019)
column_names_9019, datatype_9019, rec_type_values_9019, mappings_9019, join_conditions_9019, description_9019 = process_csv(
    csv_string_9019)

# Read the 9019 CSV and process it
csv_string_9019_transpose = read_csv_from_s3(s3_bucket, account_transpose_9019)
column_names_9019_transpose, datatype_9019_transpose, rec_type_values_9019_transpose, mappings_9019_transpose, join_conditions_9019_transpose, description_9019_transpose = process_csv(
    csv_string_9019_transpose)

# Create a dictionary for column descriptions (if available), defaulting to "No description available" if missing
column_descriptions_9000 = {col: description_9000[i] if description_9000[i] else "No description available"
                       for i, col in enumerate(column_names_9000)}
column_descriptions_9004 = {col: description_9004[i] if description_9004[i] else "No description available"
                       for i, col in enumerate(column_names_9004)}
column_descriptions_9005 = {col: description_9005[i] if description_9005[i] else "No description available"
                       for i, col in enumerate(column_names_9005)}
column_descriptions_9005_transpose = {col: description_9005_transpose[i] if description_9005_transpose[i] else "No description available"
                       for i, col in enumerate(column_names_9005_transpose)}
column_descriptions_9019 = {col: description_9019[i] if description_9019[i] else "No description available"
                       for i, col in enumerate(column_names_9019)}
column_descriptions_9019_transpose = {col: description_9019_transpose[i] if description_9019_transpose[i] else "No description available"
                       for i, col in enumerate(column_names_9019_transpose)}
column_descriptions_output = {col: description_output[i] if description_output[i] else "No description available"
                       for i, col in enumerate(column_names_output)}
# Convert to DataFrame for input
df_input = pd.DataFrame({col: [] for col in column_names_9000})
...
composite_key_columns = [col for col, mapping in zip(
    column_names_output, mappings) if mapping != 'Direct Mapping']
composite_key_columns_9005 = [col for col, mapping in zip(
    column_names_9005_transpose, mappings) if mapping != 'Direct Mapping']
composite_key_columns_9019 = [col for col, mapping in zip(
    column_names_9019_transpose, mappings) if mapping != 'Direct Mapping']
...
def handle_datatype(value):
    ...
    if isinstance(value, str) and len(value.strip()) > 0:
        ...
        return 'UNKNOWN (empty field)'


# Process the datatypes using the handle_datatype function
column_datatypes_9000 = [handle_datatype(val) for val in datatype_9000]
...
schema_fields_9000 = [{"name": col, "type": column_datatypes_9000[i], "description": column_descriptions_9000.get(col, "No description available")}  for i, col in enumerate(column_names_9000)]
schema_fields_output = [{"name": col, "type": column_datatypes_output[i], "description": column_descriptions_output.get(col, "No description available")} for i, col in enumerate(column_names_output)]
schema_fields_9004 = [{"name": col, "type": column_datatypes_9004[i], "description": column_descriptions_9004.get(col, "No description available")} for i, col in enumerate(column_names_9004)]
schema_fields_9005 = [{"name": col, "type": column_datatypes_9005[i], "description": column_descriptions_9005.get(col, "No description available")} for i, col in enumerate(column_names_9005)]
schema_fields_9019 = [{"name": col, "type": column_datatypes_9019[i], "description": column_descriptions_9019.get(col, "No description available")} for i, col in enumerate(column_names_9019)]
schema_fields_9005_transpose = [{"name": col, "type": column_datatypes_9005_transpose[i], "description": column_descriptions_9005_transpose.get(col, "No description available")} for i, col in enumerate(column_names_9005_transpose)]
schema_fields_9019_transpose = [{"name": col, "type": column_datatypes_9019_transpose[i], "description": column_descriptions_9019_transpose.get(col, "No description available")} for i, col in enumerate(column_names_9019_transpose)]
...
columns_9005_to_intermediate = [col for col in df_9005_transpose.columns if col in df_lineage_9005.columns]
columns_9019_to_intermediate = [col for col in df_9019_transpose.columns if col in df_lineage_9019.columns]
columns_9005_to_staging = [col for col in df_9005.columns if col in df_9005_transpose.columns]
columns_9019_to_staging = [col for col in df_9019.columns if col in df_9019_transpose.columns]
    ...
    "facets": {
        ...
        "columnLineage": {
            ...
            "fields": {
                ...
                } for col in columns_9005_to_staging
            }
        }
    }
}
        ...
        "columnLineage": {
            ...
            "fields": {
                col: {
                    ...
                    "transformationDescription": "Transferred without modification",
                    "transformationType": "IDENTITY"
                } for col in columns_9019_to_staging
            }
        }
    }
}
...
# Define input datasets for lineage checking
input_datasets = {
    "staging": ["Account-9000-Staging", "Account-9004-Staging", "Account-9005-Staging", "Account-9019-Staging"],
    "transpose": ["9005-transpose", "9019-transpose"],
    "intermediate": ["intermediate-9000-extract", "intermediate-9004-extract", 
                     "intermediate-9005-extract", "intermediate-9019-extract"]
}

def generate_composite_key_lineage(namespace, dataset, composite_key_columns, join_conditions):
    ...
    for col, join_condition in zip(composite_key_columns, join_conditions):
        if join_condition:
            ...
            for field in join_condition.split(','):
                field = field.strip()
                # Check lineage for the specific dataset
                input_fields.append({
                    "namespace": namespace,
                    "name": dataset,
                    "field": field
...
lineage_transpose_9005["facets"]["columnLineage"]["fields"] = integrate_composite_key_lineage(
    lineage_transpose_9005["facets"]["columnLineage"]["fields"],
    generate_composite_key_lineage(namespace, "Account-9005-Staging", composite_key_columns_9005, join_conditions_9005_transpose)
)
lineage_transpose_9019["facets"]["columnLineage"]["fields"] = integrate_composite_key_lineage(
    lineage_transpose_9019["facets"]["columnLineage"]["fields"],
    generate_composite_key_lineage(namespace, "Account-9019-Staging", composite_key_columns_9019, join_conditions_9019_transpose)
)
lineage_output["facets"]["columnLineage"]["fields"] = integrate_composite_key_lineage(
    lineage_output["facets"]["columnLineage"]["fields"],
    generate_composite_key_lineage(namespace, input_datasets["intermediate"], composite_key_columns, join_conditions)
)
lineage_event_9000 = {
    "eventType": "COMPLETE",
